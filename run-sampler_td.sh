...
# and uses random seeds for each run. The TD_sample argument is set to True.
# Runs for all environments with two samplers.
# =====================================
...
do
    ...
    do
        ...
        echo "Starting main.py for ENV_NAME=${ENV_NAME}, ALGO=${ALGO}, SAMPLER=${SAMPLER}, SAMPLE_NUM=${SAMPLE_NUM}, seed=${SEED} on device=${DEVICE}"
        ...
        python main.py --group "${ALGO}" --rl_type online --env_name "${ENV_NAME}" \
            ...
            --sample_num "${SAMPLE_NUM}" --seed "${SEED}" --TD_sample True > "${LOG_FILE}" 2>&1 &

        # Capture PID of the background process
        PID=$!
        echo "Started ENV_NAME=${ENV_NAME}, ALGO=${ALGO}, SAMPLER=${SAMPLER}, SAMPLE_NUM=${SAMPLE_NUM} with seed=${SEED} and PID=${PID} on device=${DEVICE}. Logging to ${LOG_FILE}."
